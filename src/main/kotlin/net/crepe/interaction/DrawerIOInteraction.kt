...
import net.crepe.components.DrawerContainerComponent.Companion.getComponentType
...
class DrawerIOInteraction : SimpleBlockInteraction() {
    ...
    override fun interactWithBlock(
        ...
        val drawerContainerComponent = blockRef.store.ensureAndGetComponent(blockRef, DrawerContainerComponent.getComponentType())
        val drawerUpgradableComponent = blockRef.store.getComponent(blockRef, DrawerUpgradableComponent.getComponentType())
        ...
        when (action) {
            DrawerAction.Extract -> extractItem(blockRef, drawerContainerComponent, cmdBuffer, player, movementState, ctx, pos)
            DrawerAction.Insert  -> if (item != null) insertItem(blockRef, drawerContainerComponent, drawerUpgradableComponent, ctx, item, pos) else {
                ctx.state.state = InteractionState.Failed
                return
            }
        }
    ...
    fun insertItem(
        ...
        blockRef.store.replaceComponent(blockRef, getComponentType() ,component)
    }
